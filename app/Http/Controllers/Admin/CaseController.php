...
class CaseController extends Controller
{
    ...
    public function invitation_mediate($id)
    {
        ...
        'users.country as usercountry', 'mediation_case.poc_name as userpname', 'mediation_case.poc_email as userpemail', 'mediation_case.poc_contact as userpcontact')
         ...
         // Added for batch name identification
          $batch_info = Batch::select("batch_name")->join("mediation_case", "mediation_case.batch_id", "=", "batch.id")
            ->where("mediation_case.id", "=", $id)
            ->where("mediation_case.batch_id", "=", $data["case"]->batch_id)
            ->first();

           

        // Added for respondant name 
        $rp_name = InvoledUser::select("name")
            ->where("userPlanId", "=", $id)
            ->where('isClaimant', 1)
            ->first();

        $data['primary_rp_name'] = $rp_name['name'];
        ...
        if($data["case"]->batch_id == 63){
            ...
            if($data['case']->bulk_flag == 1){
                if (str_contains(strtolower($batch_info['batch_name']), "svatantra")) {
                    $pdf = PDF::loadView('pdf.invitation_mediation_svatantra', $data, [], [
                        'title' => 'ITM' . $id,
                        'mode' => 'utf-8',
                        ...
                        if ($value->isClaimant > 0) {
                            ...
                            if ($value->userPhone != null) {
                                ...
                                if($MedCasedata->PayLink !="" and $MedCasedata->restructure_offer_1 !="" and $responding_partyforbot->userPhone == $value->userPhone){
                                    ...
                                    $varjson = ['initiating' => ($pone->organization != null) ? $pone->organization : $pone->name, 'caseid' => "M" . sprintf("%06d", $id)];
                                    $var = ['-ip-', '-cid-'];
    ...
    public function bulkUpload(Request $request)
    {
        ...
        if ($selectCsv == null) {
            ...
            foreach ($csv as $k => $value) {
                ...
                // Add zoom data : 04/02/2026
                $data['zoom_date'] = $value[26];
                $data['zoom_time'] = $value[27];
                $data['zoom_link'] = $value[28];
                $data['enterprise'] = $value[29];
                // Add zoom data : 04/02/2026
        ...
        foreach ($caseid as $key => $value) {
            ...
            if (isset($invitation->file_name) && $invitation->file_name != null) {
                ...
                if (File::exists($exist_file)) { 
                    ...
                    if(File::exists($exist_file_local)){
    ...
    public function ShowMomSessionData(Request $request) {
        ...
        $mediator = Mediators_mediation_cases_status::select("first_name", "last_name")->join("users", "users.id", "=", "mediators_mediation_cases_status.mediator_id")
            ...
            // $data["case"] = MedCase::select('id', 'discussion', 'otherRespondentDetails')->where("id", "=", $request['caseid'])->first();
            // $data["party"] = InvoledUser::select('user_involved_in_agreement.*', 'users.address as useraddress', 'users.address1 as useraddress1', 'users.pincode as userpincode', 'users.city as usercity', 'users.state as userstate', 
            // 'users.country as usercountry', 'mediation_case.poc_name as userpname', 'mediation_case.poc_email as userpemail', 'mediation_case.poc_contact as userpcontact')
            //     ->leftJoin("users", "users.id", "=", "user_involved_in_agreement.userId")
            //     ->leftJoin("mediation_case", "mediation_case.id", "=", "user_involved_in_agreement.userPlanId")
        ...
        if ($result->save()) {
            ...
            foreach ($request->session_party_ids as $party_id) {
                ...
                if($request->zoomChoice == "manual") {
                    $this->sned_session($request->zoomId, $result->case_id, $party->userEmail, $party->name, $display_date_time, $party->userPhone, "Party");
                } else {
                    $this->sned_session_invitation($request->zoomId, $result->case_id, $party->userEmail, $party->name, $display_date_time, $party->userPhone, $request->zoomLink, "Party");
                }
            ...
            if ($mediator) {
                ...
                // $access = Whatsapp::sendWamessage($dwa1);
                if($request->zoomChoice == "manual") {
                    $is_send = $this->sned_session($request->zoomId, $request->CaseId, $mediator->email, $mediator->username, $display_date_time, $mediator->mobile_number, "Mediator");
                } else if($request->zoomChoice == "direct") {
                /**** Zoom Invitation ************/
                    $is_send = $this->sned_session_invitation($request->zoomId, $request->CaseId, $mediator->email, $mediator->username, $display_date_time, $mediator->mobile_number, $request->zoomLink, "Mediator");
                /**** Zoom Invitation ************/
                }
            }
        }
                ...
                if (isset($data['emailtrck']['track_data'])) {
                    foreach ($data['emailtrck']['track_data'] as $etrck) {
                        if ($etrck->event  == "delivered") {
                            ...
                            $caseinfo['invers'] = "The email was successfully delivered and no 'return to sender' message, 'bounce back' message or 'error' message was received from the mail server";
