...
class DashboardController extends Controller
{
    ...
    public function getNotifications()
    {
        ...
        $noficationdata = Notification::mediatornotificationDataAPI($userId);
        ...
        if(count($noficationdata) > 0) {
            ...
            foreach ($noficationdata as $key => $values) {
                ...
                if(!empty($values->case_id)){
                    ...
                    $data[$key]['case_id'] ='CID' . sprintf('%06d', $values->id);
    ...
    public function getCaseNotifications(Request $request)
    {
        try{
            ...
            $noficationdata = Notification::mediatornotificationbyctgry($userId, 1);
    ...
    public function getDocsNotifications(Request $request)
    {
        try{
            $token = $request->cookie('auth_token');
            if (!$token) {
            ...
            $noficationdata = Notification::mediatornotificationbyctgry($userId, 2);
    ...
    public function getSessionNotifications(Request $request)
    {
        try{
            $token = $request->cookie('auth_token');
            if (!$token) {
            ...
            $noficationdata = Notification::mediatornotificationbyctgry($userId, 3);
    ...
    public function getAccountsNotifications(Request $request)
    {
        try{
            $token = $request->cookie('auth_token');
            if (!$token) {
            ...
            $noficationdata = Notification::mediatornotificationbyctgry($userId, 4);
