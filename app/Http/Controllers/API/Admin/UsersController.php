...
class UsersController extends Controller
{
    ...
    public function mediatorRejected(Request $request)
    {
        ...
        $users = User::getMediatorsRejected($role, $start, $length, $search, $columnName, $sortOrder, $category);
    ...
    public function statusChangeApprove(Request $request)
    {
        ...
        if ($user->role == 0 && $status == 1) {
            $err = SendGrid::send($d, $user->email, env('UK_L23_USER_ACCOUNT_ACTIVATION', ''));
        } else if ($status == 1) {
            SendGrid::send($d, $user->email, env('UK_L24_MEDIATOR_ACCOUNT_ACTIVATION', ''));
        }
        $user->save();
