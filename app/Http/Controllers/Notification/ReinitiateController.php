...
class ReinitiateController extends Controller
{
    ...
    public function reinitiate()
    {
        ...
        $allData = DB::table('reinitiate_noti_janarthanan')->where('is_whtsapp_sent', 0)->where('is_pdf_sent', 0)->limit(100)->get();
        ...
        foreach($allData as $data) {
            ...
            if($access1) {
                $is_update_wa = DB::table('reinitiate_noti_janarthanan')->where('caseid', $data->caseid)->update(['is_whtsapp_sent' => 1]);
            ...
            if($access2) {
                $is_update_pdf = DB::table('reinitiate_noti_janarthanan')->where('caseid', $data->caseid)->update(['is_pdf_sent' => 1]);
